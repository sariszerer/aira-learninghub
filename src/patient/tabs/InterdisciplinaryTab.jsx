...
import { Btn, Card, EmptyNote, Eyebrow, Tabs } from "../../ui/index.js";
...
function InterdisciplinaryTab({ child, meetings, users, onAddMeeting, onEditMeeting, currentUser, documents, onAddDocument }) {
  ...
  const [seccion, setSeccion] = useState("minutas");
  ...
  // Las dos secciones en pestañas y no una debajo de la otra.
  //
  // Apiladas funcionaban mientras estaban vacías. Con una minuta de veinte
  // líneas dentro, Pautas de Crianza queda a una pantalla y media de scroll y
  // deja de existir: no se ve que esté ahí, ni que tenga contenido.
  //
  // El recuento va en la pestaña por lo mismo. Sin él, "Minutas" y "Minutas 3"
  // se leen igual, y hay que entrar para saber si hay algo.
  const pestanas = [
    { id: "minutas", label: `Minutas${childMeetings.length ? ` · ${childMeetings.length}` : ""}` },
  ];
  if (canSeePautas) {
    pestanas.push({
      id: "pautas",
      label: `Pautas de Crianza${pautasSessions.length ? ` · ${pautasSessions.length}` : ""}`,
    });
  }
  ...
  const seccionPautas = (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
        <Eyebrow style={{ marginBottom: 0 }}>Sesiones con madres</Eyebrow>
            ...
            <Card key={d.id} style={{ padding: "12px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                ...
                <div style={{ fontSize: 12, color: T.inkFaint }}>
                  {users.find((u) => u.id === d.authorId)?.name || "—"}
                </div>
              </div>
  ...
  const seccionMinutas = (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
        <Eyebrow style={{ marginBottom: 0 }}>Reuniones registradas</Eyebrow>
  ...
  return (
    <div>
      {/* Con una sola pestaña no se pinta el riel: un control para elegir
          entre una cosa es ruido. */}
      {pestanas.length > 1 && (
        <Tabs tabs={pestanas} activo={seccion} onCambiar={setSeccion} />
      )}

      {seccion === "pautas" && canSeePautas ? seccionPautas : seccionMinutas}
